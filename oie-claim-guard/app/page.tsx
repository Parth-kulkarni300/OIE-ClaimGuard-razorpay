  ...
  const updateStatus = async (status: ClaimStatus) => {
    ...
    if (status === 'Approved') {
      ...
      try {
        ...
        setClaims(claims.map((claim) => claim.id === selected.id ? { ...claim, status } : claim));
        alert("An agent will be sent for manual inspection.");
      } catch (e) {
        console.error(e);
      }
    } else {
      ...
      {tab === 'analytics' ? (
        ...
        <>
          ...
          {filteredClaims.length === 0 ? (
            ...
            <GlassCard className="p-5 sm:p-7"><div className="flex flex-col justify-between gap-4 border-b border-white/8 pb-5 sm:flex-row sm:items-start"><div><div className="flex items-center gap-2"><span className="eyebrow">Claim detail</span><Status status={selected.status} /></div><h2 className="mt-3 text-2xl font-semibold tracking-tight text-white">{selected.claimId}</h2><p className="mt-1 text-sm text-slate-400">Submitted {selected.submitted} by {selected.name}</p></div></div><div className="mt-6 grid gap-6 lg:grid-cols-[0.92fr_1.08fr]"><div><div className="damage-image"><img src={selected.image} alt="Uploaded vehicle damage evidence" /><div className="image-label"><ImagePlus size={14} /> Evidence image</div></div><div className="mt-4 grid grid-cols-3 gap-2"><Info label="Customer" value={selected.name} /><Info label="Vehicle no." value={selected.vehicle} /><Info label="Policy" value="Comprehensive" /></div><div className="mt-4"><Info label="Customer Description" value={selected.description} /></div></div><div><div className="flex items-center justify-between"><h3 className="font-medium text-white">AI triage report</h3><div className="flex items-center gap-2"><button onClick={downloadPDF} className="flex items-center gap-1 text-[11px] text-cyan hover:text-white px-2 py-1 rounded border border-cyan/30 transition-colors"><Download size={12}/> PDF Audit</button><span className={`risk-pill ${selected.risk > 75 ? 'risk-high' : selected.risk > 40 ? 'risk-medium' : 'risk-low'}`}><Gauge size={13} /> {selected.risk}% risk</span></div></div><div className="mt-4 rounded-2xl border border-white/8 bg-white/[0.025] p-5"><p className="text-sm leading-6 text-slate-300">{selected.reasoning}</p><div className="mt-5 grid grid-cols-3 gap-3"><div className="report-stat"><span>Severity</span><strong>{selected.severity || 'UNKNOWN'}</strong></div><div className="report-stat"><span>Visual match</span><strong>{selected.risk > 75 ? 'Low' : 'High'}</strong></div><div className="report-stat"><span>Rec. Payout</span><strong className="text-emerald">₹{selected.recommendedPayout?.toLocaleString() || 5000}</strong></div></div></div><div className="mt-5 flex flex-col gap-3 sm:flex-row">{selected.status === 'Pending Review' || selected.status === 'Flagged' ? (selected.severity === 'SEVERE' ? <button className="primary-button flex-1" onClick={() => updateStatus('Inspection Required')}><Users size={16} /> Send Agent for Inspection</button> : <><button className="approve-button flex-1" onClick={() => updateStatus('Approved')}><BadgeCheck size={16} /> Approve ₹{selected.recommendedPayout?.toLocaleString() || 5000}</button><button className="secondary-button flex-1" onClick={() => updateStatus('Inspection Required')}><Users size={16} /> Send Agent</button></>) : selected.status === 'Inspection Required' ? <div className="rounded border border-cyan/20 bg-cyan/10 p-3 w-full flex items-center justify-center gap-2 text-sm text-cyan"><Users size={16} /> Field Agent Dispatched for Inspection</div> : <div className="rounded border border-emerald/20 bg-emerald/10 p-3 w-full flex items-center justify-center gap-2 text-sm text-emerald"><BadgeCheck size={16} /> Payout Initiated via Razorpay <span className={`ml-2 text-[10px] px-2 py-0.5 rounded-full ${selected.payoutStatus === 'Processed' ? 'bg-emerald/20 text-emerald' : 'bg-amber/20 text-amber'}`}>{selected.payoutStatus || 'Processing'}</span></div>}</div></div></div></GlassCard>
          ) : null}
          </div>
        </>
      )}
